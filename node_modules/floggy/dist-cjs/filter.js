...
const Level = (0, tslib_1.__importStar)(require("./level"));
...
function parse(defaults, pattern) {
    ...
    if (!patterns.length) {
        return [(0, Either_1.left)(createInvalidPattern(pattern, 'There must be at least one pattern present.'))];
    }
...
function parseOne(criteriaDefaults, pattern) {
    ...
    if (pattern === '') {
        return (0, Either_1.left)(createInvalidPattern(originalInput));
    }
    ...
    const ex = parts[parts.length - 2] === '' && ((_a = parts[parts.length - 1]) === null || _a === void 0 ? void 0 : _a[0]) === '*';
    if (ex) {
    ...
    if (levelm) {
        if (levelm.type === 'wildcard') {
            ...
            if (levelm.dir === '+')
                ...
                (0, utils_1.casesHandled)(levelm.dir);
        }
    }
    /**
     * Check For Errors
    ...
    if (path.value !== null) {
        const invalidPathPartNames = path.value
            ...
            .slice(1) // root
            .filter((pathPart) => !data_1.validPathSegmentNameRegex.exec(pathPart));
        if (invalidPathPartNames.length) {
            return (0, Either_1.left)(createInvalidPattern(originalInput, `Path segment names must only contain ${String(data_1.validPathSegmentNameRegex)}.`));
        }
    }
...
function createInvalidPattern(pattern, hint) {
    return (0, utils_1.createContextualError)(`Invalid filter pattern: "${pattern}${hint ? `. ${hint}` : ''}"`, {
        pattern,
        hint
    });
}
...
function renderSyntaxError(input) {
    ...
    if (!multipleInputs) {
        // eslint-disable-next-line
        const e = (0, utils_1.getLeft)(badOnes[0]);
